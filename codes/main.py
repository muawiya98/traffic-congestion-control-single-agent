...
class SUMO_ENV:
    ...
    def run(self):
        ...
        for i in range(5):
            ...
            # Normal scenario
            if i ==0:self.controller.run_agents(Random=False,with_agents=True)
            # Rely on sumo decisions
            elif i ==1:self.controller.run_agents(Random=False,with_agents=False)
            # Give a random action
            elif i ==2:self.controller.run_agents(Random=True,with_agents=False)
            self.exit()
